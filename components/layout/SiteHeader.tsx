              ...
              <ul className="flex items-center gap-7 xl:gap-9">
                ...
                {headerPrimaryLinks.map((link, index) => (
                  <li key={link.href} {...reveal(index + 2)}>
                    <a
                      href={link.href}
                      className="inline-flex rounded-control py-2 text-[0.875rem] leading-5 font-medium whitespace-nowrap text-paper/90 transition-colors duration-150 hover:text-paper group-data-[condensed=true]:text-ink group-data-[condensed=true]:hover:text-brand"
                    >
                      {link.label}
                    </a>
                  </li>
